...
@ccclass
export default class MofoEnemy extends EnemyBaseClass {
    ...
    start() {
        this.totalHp = ConfigManager.getInstance().getConfigById(EnemyConfigContainer, 11).hp / Math.pow(2, this.generation);
        ...
        this.schedule(this.changeMoveState, 1,cc.macro.REPEAT_FOREVER,0.1);
    ...
    private effectBySword() {
        ...
        switch (swordData.id) {
            ...
            case "7":
                ...
                if (rate6 < random6) {
                    this.unschedule(this.changeMoveState.bind(this));
                    this.isMove = false;
                    this.scheduleOnce(this.changeMoveState.bind(this), 2);
                }
                break;
            case "8":
                // 绯云剑 TODO
    ...
    update(dt) {
        if (this.isMove) {
            this.node.x += this.speedX * dt*this.speed;
            this.node.y += this.speedY * dt*this.speed;
        }
    }
}
